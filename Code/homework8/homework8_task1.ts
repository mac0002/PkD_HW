...
const check_out_book =
    (book_item: Book_item) => [get_title(book_item), get_isbn(book_item), true];

// Returns a book with the same title and ISBN, but marked as checked in.
const check_in_book =
    (book_item: Book_item) => [get_title(book_item), get_isbn(book_item), false];
...
function add_book(book: Book_item, library: Library): Library {
    ...
    function insert(lib: Library): Library {
        // variant: length(lib)
        return is_null(lib)
               ? list(book)
...
function find_book(library: Library, isbn: ISBN): Book_item | null {
    function get(lib: Library): Book_item | null{
        // variant: length(lib)
        return is_null(lib)
               ? null
 ...
 *     as checked out.
 */
function check_out(library: Library, isbn: ISBN): Library {
 ...
 *     as checked in.
 */
function check_in(library: Library, isbn: ISBN): Library {
